...
private:
    ...
    unordered_map<string, deque<string>> records;               // maintains a window of records relative to the same entity_id
        ...
        if ((records.find(entity_id)->second).size() == records_win_size) {
            for (string r : records.find(entity_id)->second) {  // read the sequence of states from the records in the window
                if (state_position == 0)
                    ...
                    states_sequence.insert(states_sequence.end(), r.substr(r.find(split_regex) + 1, r.size()));
            }
